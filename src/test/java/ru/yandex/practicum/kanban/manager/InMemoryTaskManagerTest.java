...
class InMemoryTaskManagerTest extends TaskManagerTest<InMemoryTaskManager> {
    ...
    void updateEpicDuration() {
        Epic epicTime1 = new Epic("Epic update epic duration", "Description");
        taskManager.addTask(epicTime1);

        assertNull(taskManager.getEpicById(epicTime1.getId()).getStartTime(), "ошибка добавления эпика.");
        SubTask subTask1 = new SubTask(LocalDateTime.now().plusMinutes(2), Duration.ofMinutes(1), "Subtask epic duration", "Description", epicTime1.getId());
        ...
        SubTask subTask2 = new SubTask(LocalDateTime.now(), Duration.ofMinutes(1), "subtask now", "Description", epicTime1.getId());
    ...
    void getPrioritizedTasks() {
        ...
        Task taskTime2 = new Task(LocalDateTime.now().plusMinutes(2), Duration.ofSeconds(59), "Задача со временем3", "Проверка записи в лист");
        taskManager.addTask(taskTime2);
        Task taskTime1 = new Task(LocalDateTime.now().plusMinutes(1), Duration.ofSeconds(59), "Задача со временем2", "Проверка записи в лист");
        taskManager.addTask(taskTime1);
        Task taskTime0 = new Task(LocalDateTime.now(), Duration.ofSeconds(59), "Задача со временем1", "Проверка записи в лист");
    ...
    void validateTaskInTimeAdd() {
        Task taskTime1 = new Task(LocalDateTime.of(2022, 8, 25, 10, 0), Duration.ofHours(1), "Задача со временем", "Проверка записи в лист  c startTime");
        taskManager.addTask(taskTime1);
        int taskId1 = taskTime1.getId();

        Task taskTime2 = new Task(LocalDateTime.of(2022, 8, 25, 11, 1), Duration.ofHours(1), "Задача со временем2", "Проверка записи в лист c startTime");
        taskManager.addTask(taskTime2);
